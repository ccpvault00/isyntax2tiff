...
from datetime import timezone #ziyu
...
class WriteTiles(object):
    ...
    # New helper method to handle attributes safely by ziyu
    def safe_get(self, obj, attr, default=None):
        """Safely get attribute from an object without raising RuntimeError."""
        try:
            return getattr(obj, attr)
        except RuntimeError:  # Catch the specific error related to SinglePodContainer
            return default
    ...
    def get_metadata_sdk_v2(self):
        ...
        return {
            ...
            "Scanner rack number":
                self.safe_get(pe_in, "scanner_rack_number", None), #ziyu
                # pe_in.scanner_rack_number
            "Scanner rack priority":
                pe_in.scanner_rack_priority,
            "Scanner slot number":
                self.safe_get(pe_in, "scanner_slot_number", None), #ziyu
        ...
        if image_type == "WSI":
            ...
            for resolution in range(self.num_derived_levels(img)):
                ...
                if resolution == 0:
                    self.size_x = level_size_x
                    self.size_y = level_size_y
            self.label_x = self.get_size(dim_ranges[0]) + 1 #ziyu
            self.label_y = self.get_size(dim_ranges[1]) + 1 #ziyu
            self.macro_x = self.get_size(dim_ranges[0]) + 1 #ziyu
            self.macro_y = self.get_size(dim_ranges[1]) + 1 #ziyu
    ...
    def acquisition_datetime(self):
        ...
        try:
            ...
            return datetime(2025, 2, 25, 15, 30, 45, tzinfo=timezone.utc) #ziyu, hardcode the timestamp
            # return parse(timestamp) #ziyu
